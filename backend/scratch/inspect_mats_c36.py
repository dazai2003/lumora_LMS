"""
Inspect materials and material_flags for course 36
"""
import os, sys
...
for m in mats:
    print(f"  Material #{m[0]}: '{m[2]}'")

flags = db.execute(text("""
    SELECT mf.id, mf.material_id, m.title, mf.comment, mf.student_id 
    FROM material_flags mf 
    JOIN materials m ON mf.material_id = m.id 
    WHERE m.course_id = 36 
    ORDER BY mf.id;
""")).fetchall()
print(f"\nCourse 36 material flags ({len(flags)}):")
for f in flags:
    print(f"  Flag #{f[0]} | Material #{f[1]} ('{f[2]}') | Comment: {f[3]} | Student: {f[4]}")
