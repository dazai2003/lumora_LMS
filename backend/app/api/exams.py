...
def get_pending_teacher_reviews(
    ...
    if status and status.lower() != "all":
        ...
        if st in ["pending", "action_needed", "needs_review"]:
            query = query.filter(ALStudentSubmission.status.in_(["ai_graded", "submitted"]))
        elif st == "in_progress":
            query = query.filter(ALStudentSubmission.status == "in_progress")
        else:
            query = query.filter(ALStudentSubmission.status == st)
    else:
        # Default 'all' for teacher queue only includes completed/submitted attempts
        query = query.filter(ALStudentSubmission.status.in_(["ai_graded", "submitted", "teacher_verified", "graded"]))

    return query.order_by(ALStudentSubmission.submitted_at.desc().nullslast(), ALStudentSubmission.id.desc()).all()
