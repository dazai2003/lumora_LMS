...
/* ─────────────────────────────────────────────────────────────
 * 2. ENTERPRISE GEMINI-GRADE CLASS SUBSCRIPTION & BILLING SUITE
 * ───────────────────────────────────────────────────────────── */
export function SubscriptionsReceiptsModal({ onClose, user }: { onClose: () => void; user: any }) {
  const [subscriptions, setSubscriptions] = useState<SubscriptionResponse[]>([]);
  const [transactions, setTransactions] = useState<PaymentResponse[]>([]);
  const [loadingBilling, setLoadingBilling] = useState(true);
  const [selectedReceipt, setSelectedReceipt] = useState<PaymentResponse | null>(null);
  ...
  const metrics = useMemo(() => {
    const totalCommitment = subscriptions.length * 2500;
    const activeSubs = subscriptions.filter(s => s.status === "active");
    const overdueSubs = subscriptions.filter(s => s.status === "overdue");

    const dates: Date[] = subscriptions
      .filter(s => Boolean(s.current_period_end))
      .map(s => new Date(s.current_period_end));

    let nextRenewalDate: Date | null = null;
    if (dates.length > 0) {
      nextRenewalDate = new Date(Math.min(...dates.map(d => d.getTime())));
    }

    const now = new Date();
    const daysRemaining = nextRenewalDate ? Math.max(0, Math.ceil((nextRenewalDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))) : null;

    let standingLabel = "Active & In Good Standing";
    let standingColor = "#10B981";
    let standingBg = "rgba(16, 185, 129, 0.12)";
    let standingBorder = "rgba(16, 185, 129, 0.3)";

    if (overdueSubs.length > 0) {
      standingLabel = "Payment Required / Fee Overdue";
      standingColor = "var(--error)";
      standingBg = "rgba(239, 68, 68, 0.12)";
      standingBorder = "rgba(239, 68, 68, 0.3)";
    } else if (daysRemaining !== null && daysRemaining <= 5) {
      standingLabel = "Tuition Fee Due Soon";
      standingColor = "#D97706";
      standingBg = "rgba(217, 119, 6, 0.12)";
      standingBorder = "rgba(217, 119, 6, 0.3)";
    }

    return {
      totalCommitment,
      activeSubsCount: activeSubs.length,
      nextRenewalDateStr: nextRenewalDate ? nextRenewalDate.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "N/A",
      daysRemaining,
      standingLabel,
      standingColor,
      standingBg,
  ...
  return (
    <Modal title="Subscription Membership & Billing Hub" onClose={onClose} maxWidth="840px">
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }} className="animate-fade-in">
        {loadingBilling ? (
          ...
          <>
            {/* HERO BANNER: Gemini Pro-Style Class Membership Status */}
            <div style={{
              padding: "1.4rem 1.6rem", borderRadius: "var(--radius-lg)",
              background: "linear-gradient(135deg, rgba(37,99,235,0.08) 0%, rgba(124,58,237,0.12) 100%)",
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "1rem" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.3rem" }}>
                    <span className="badge" style={{ background: "rgba(37,99,235,0.15)", color: "var(--accent-primary)", border: "1px solid rgba(37,99,235,0.3)", fontSize: "0.725rem", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                      Class Pass Passholder
                    </span>
                    <span style={{
                      fontSize: "0.75rem", fontWeight: 700, padding: "0.15rem 0.55rem", borderRadius: "100px",
                      background: metrics.standingBg, color: metrics.standingColor, border: `1px solid ${metrics.standingBorder}`
                    }}>
                  ...
                  <h3 style={{ fontSize: "1.35rem", fontWeight: 800, margin: 0, color: "var(--text-primary)" }}>
                    {subscriptions.length > 0 ? `${subscriptions.length} Enrolled Subject Classes` : "No Active Class Subscriptions"}
                  </h3>
                </div>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: "1.6rem", fontWeight: 800, color: "var(--text-primary)", lineHeight: 1.2 }}>
                    LKR {metrics.totalCommitment.toLocaleString()} <span style={{ fontSize: "0.85rem", fontWeight: 500, color: "var(--text-muted)" }}>/ mo</span>
                  </div>
                </div>
              </div>

              {/* Next Renewal Date Summary Line */}
              <div style={{
                display: "flex", justifyContent: "space-between", alignItems: "center",
                padding: "0.65rem 0.9rem", borderRadius: "var(--radius-md)", background: "var(--bg-card)",
                border: "1px solid var(--border-subtle)", fontSize: "0.825rem", flexWrap: "wrap", gap: "0.5rem"
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "var(--text-secondary)" }}>
                  <SvgIcon name="calendar" size={16} style={{ color: "var(--accent-primary)" }} />
                  <span>Next Tuition Renewal: <strong>{metrics.nextRenewalDateStr}</strong></span>
                </div>

                {metrics.daysRemaining !== null && (
                  <span style={{ fontSize: "0.78rem", fontWeight: 700, color: metrics.daysRemaining <= 5 ? "#D97706" : "var(--accent-primary)" }}>
                    {metrics.daysRemaining === 0 ? "Renews Today" : `Renews in ${metrics.daysRemaining} days`}
                  </span>
                )}
              </div>
            </div>

            {/* SECTION 1: Active Class Pass Cards Grid */}
            <div>
              <h4 style={{ margin: "0 0 0.75rem 0", fontSize: "0.95rem", fontWeight: 700, color: "var(--text-primary)", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                ...
                <span>Active Subject Class Subscriptions ({subscriptions.length})</span>
              </h4>

              {subscriptions.length > 0 ? (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "1rem" }}>
                  {subscriptions.map(sub => (
                    <div key={sub.id} className="card shadow-sm" style={{
                      padding: "1.1rem", borderRadius: "var(--radius-md)", background: "var(--bg-card)",
                      border: "1px solid var(--border-subtle)", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: "0.75rem"
                    }}>
                      <div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.4rem" }}>
                          <span style={{ fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--accent-primary)", background: "rgba(37,99,235,0.1)", padding: "2px 6px", borderRadius: "4px" }}>
                            Tuition Class Pass
                          </span>
                          <span style={{
                            fontSize: "0.7rem", fontWeight: 700, textTransform: "capitalize",
                            color: sub.status === "active" ? "#10B981" : "var(--error)"
                          }}>
                            {sub.status}
                          </span>
                        </div>

                        <h5 style={{ margin: "0 0 0.25rem 0", fontSize: "0.95rem", fontWeight: 800, color: "var(--text-primary)" }}>
                          {sub.course_title}
                        </h5>
                        
                        <div style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>
                          Fee: <strong style={{ color: "var(--text-primary)" }}>LKR 2,500 / month</strong>
                        </div>
                      </div>

                      <div style={{ paddingTop: "0.6rem", borderTop: "1px solid var(--border-subtle)", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.75rem", color: "var(--text-muted)" }}>
                        <span>Due: {sub.current_period_end ? new Date(sub.current_period_end).toLocaleDateString([], { month: "short", day: "numeric" }) : "N/A"}</span>
                        <span style={{ fontWeight: 600, color: "var(--text-secondary)" }}>Monthly Auto-Renew</span>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="card empty-state" style={{ padding: "1.75rem" }}>
                  <div className="empty-state-title" style={{ fontSize: "0.95rem" }}>No enrolled paid subject classes</div>
                  <div className="empty-state-desc" style={{ fontSize: "0.8rem" }}>Browse open courses to enroll in theory classes.</div>
                </div>
              )}
            </div>

            {/* SECTION 2: Upcoming Invoice Breakdown */}
            <div className="card shadow-sm" style={{ padding: "1.2rem", borderRadius: "var(--radius-md)", background: "var(--bg-card)", border: "1px solid var(--border-subtle)" }}>
              <h4 style={{ margin: "0 0 0.75rem 0", fontSize: "0.95rem", fontWeight: 700, color: "var(--text-primary)", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                <SvgIcon name="credit-card" size={16} style={{ color: "var(--accent-primary)" }} />
                <span>Upcoming Invoice Breakdown</span>
              </h4>

              {subscriptions.length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", fontSize: "0.825rem" }}>
                  {subscriptions.map(sub => (
                    <div key={sub.id} style={{ display: "flex", justifyContent: "space-between", color: "var(--text-secondary)", borderBottom: "1px dashed var(--border-subtle)", paddingBottom: "0.4rem" }}>
                      <span>{sub.course_title} (Monthly Tuition Fee)</span>
                      <strong style={{ color: "var(--text-primary)" }}>LKR 2,500.00</strong>
                    </div>
                  ))}

                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "0.5rem", fontWeight: 800, fontSize: "0.95rem", color: "var(--text-primary)" }}>
                    <span>ESTIMATED TOTAL DUE NEXT CYCLE</span>
                    <span style={{ color: "var(--accent-primary)", fontSize: "1.1rem" }}>LKR {metrics.totalCommitment.toFixed(2)}</span>
                  </div>
                </div>
              ) : (
                <div style={{ fontSize: "0.825rem", color: "var(--text-muted)" }}>No upcoming tuition invoices scheduled.</div>
              )}
            </div>

            {/* SECTION 3: Payment History & Official Receipts Ledger */}
            <div>
              <h4 style={{ margin: "0 0 0.75rem 0", fontSize: "0.95rem", fontWeight: 700, color: "var(--text-primary)", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                <SvgIcon name="file-text" size={16} style={{ color: "var(--accent-primary)" }} />
                <span>Official Payment Receipts & Audit History</span>
              </h4>
              ...
              {transactions.length > 0 ? (
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.825rem" }}>
                    <thead>
                      <tr style={{ background: "var(--bg-card-hover)", borderBottom: "1px solid var(--border-subtle)" }}>
                        <th style={{ padding: "0.75rem 1rem", fontWeight: 700, color: "var(--text-secondary)" }}>Receipt Ref</th>
                        <th style={{ padding: "0.75rem 1rem", fontWeight: 700, color: "var(--text-secondary)" }}>Class Particulars</th>
                        <th style={{ padding: "0.75rem 1rem", fontWeight: 700, color: "var(--text-secondary)" }}>Amount Paid</th>
                        <th style={{ padding: "0.75rem 1rem", fontWeight: 700, color: "var(--text-secondary)" }}>Status</th>
                        <th style={{ padding: "0.75rem 1rem", fontWeight: 700, color: "var(--text-secondary)", textAlign: "right" }}>Receipt</th>
                      </tr>
                    </thead>
                    <tbody>
                      {transactions.map(txn => (
                        <tr key={txn.id} style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                          ...
                          <td style={{ padding: "0.75rem 1rem", fontWeight: 600 }}>{txn.course_title}</td>
                          <td style={{ padding: "0.75rem 1rem", fontWeight: 700 }}>LKR {txn.amount.toLocaleString()}</td>
                          <td style={{ padding: "0.75rem 1rem" }}>
                            <span style={{
                              fontSize: "0.75rem", fontWeight: 700,
                              color: txn.status === "completed" ? "#10B981" : txn.status === "pending" ? "#D97706" : "var(--error)",
                              textTransform: "capitalize"
                            }}>
                              {txn.status === "completed" ? "PAID & VERIFIED" : txn.status}
                            </span>
                          </td>
                          <td style={{ padding: "0.75rem 1rem", textAlign: "right" }}>
                            {txn.status === "completed" ? (
                              ...
                              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>Pending Verification</span>
                            )}
                          </td>
                        </tr>
                      ))}
                ...
                <div className="card empty-state" style={{ padding: "1.5rem" }}>
                  <div className="empty-state-title" style={{ fontSize: "0.95rem" }}>No payment receipts recorded</div>
                </div>
              )}
            </div>
          </>
